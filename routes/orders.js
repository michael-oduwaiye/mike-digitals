 ...
 * routes/orders.js
 *
 * Customer-facing order endpoints. This is what your storefront website
 * calls when a customer picks a plan and hits "Pay."
 *
 * Flow:
 *  1. POST /api/orders          -> creates a pending order, starts a Paystack
 *                                   transaction, returns a checkout URL
 *  2. GET  /api/orders/:id      -> lets the frontend poll an order's status
 *                                   (useful right after payment, before the
 *                                   webhook has necessarily landed yet)
 */
...
// Very light phone number sanity check — 11-digit Nigerian mobile format.
...
router.post("/orders", async (req, res) => {
  try {
    const { planId, mobileNumber, customerEmail } = req.body;
    ...
    // Paystack requires an email even if you don't really collect one from
    // the customer on a simple VTU storefront — a placeholder tied to the
    // order reference works fine and keeps each transaction unique.
    ...
    db.createOrder({
      ...
      fulfillmentStatus: "not_started", // not_started -> delivered -> partial -> failed
